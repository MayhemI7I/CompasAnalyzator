...
// DeleteHistoryItem удаляет запись из истории по ID
func (a *App) DeleteHistoryItem(itemID string) error {
	Log("INFO", "DeleteHistoryItem вызвана для ID: %s", itemID)
	...
	found := false
	newItems := make([]HistoryItem, 0, len(items))
	for _, item := range items {
		if item.ID == itemID {
			found = true
			Log("INFO", "Запись найдена для удаления: %s (компас: %s)", itemID, item.Compass)
		} else {
	...
	if !found {
		Log("ERROR", "Запись с ID %s не найдена для удаления", itemID)
		return fmt.Errorf("запись с ID %s не найдена", itemID)
	}

	// Сохраняем обновлённый список
	if err := a.SaveHistory(newItems); err != nil {
		Log("ERROR", "Ошибка сохранения истории после удаления: %v", err)
		return err
	}

	Log("SUCCESS", "Запись успешно удалена, ID: %s", itemID)
	return nil
}
